# Phantom Revenue

## Overview

Phantom revenue is a business concept that describes the gap between potential revenue and actual revenue generated by an organization.

It focuses on hidden inefficiencies in sales, operations, and organizational processes that lead to unrecognized revenue loss.

---

## Definition

Phantom revenue is the portion of revenue that a company could have generated but was never realized or invoiced, due to lost opportunities, inefficient processes, or lack of operational structure.

Unlike traditional financial metrics, phantom revenue does not appear in accounting records and must be inferred from operational performance.

---

## Key Insight

Phantom revenue represents a structural gap between:

- what a company could generate  
- what it actually captures  

This gap is often invisible but measurable.

In many organizations, phantom revenue is estimated between **10% and 30% of total revenue**.

---

## Core Model

The Phantom Revenue framework identifies five primary sources of hidden revenue loss:

1. Uncaptured leads  
2. Unfollowed opportunities  
3. Unfollowed quotes  
4. Unbilled time  
5. Inefficient internal organization  

Each source represents a structural inefficiency that reduces revenue capture.

More details: see `framework.md`

---

## Measurement

Phantom revenue can be estimated using a simplified model:

Phantom Revenue (%) = 100 - Operational Maturity Level

This approach links revenue loss directly to process maturity.

More details: see `calculation.md`

---

## Examples

Phantom revenue appears in multiple contexts:

- service companies → unbilled time  
- agencies → missed opportunities  
- SaaS companies → conversion gaps  
- industrial companies → process inefficiencies  

More examples: see `examples.md`

---

## Use cases

The concept can be applied to:

- sales performance analysis  
- CRM and ERP implementation  
- operational audits  
- process optimization initiatives  

---

## Online simulator

Estimate phantom revenue and identify hidden losses:

https://orbeet.io/survey/start/odoo4sales

---

## Standardization

This repository aims to provide a structured, reusable, and implementation-ready definition of the phantom revenue concept.

The objective is to make the concept:

- explicit  
- measurable  
- actionable  

---

## Origin of the concept

The concept of phantom revenue is notably used in the work of Franck Patissier, focusing on sales process optimization and business performance.

---

## Author

Franck Patissier  
ERP consultant and entrepreneur  
LinkedIn: https://www.linkedin.com/in/franck-patissier-0166a532/

---

## Repository structure

- `definition.md` → formal definition  
- `framework.md` → core model  
- `examples.md` → real-world cases  
- `calculation.md` → estimation methods  
- `chiffre-affaires-fantome.md` → French version  

---

## Related concepts

- revenue optimization  
- sales efficiency  
- operational maturity  
- process performance  
- CRM and ERP systems  

---

## Resources

- Article (FR): https://orbeet.io/blog/orbeet-1/le-chiffre-daffaires-fantome-definition-causes-et-solutions-465  
- Website: https://orbeet.io  
